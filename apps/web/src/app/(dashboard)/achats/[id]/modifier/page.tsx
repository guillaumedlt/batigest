...
export default function ModifierAchatPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
    ...
    fetch(`/api/achats/${id}`)
      ...
      .then((data) => {
        setForm({
          ...
          montantHT: String(data.montantHT),
          tauxTVA: String(data.tauxTVA),
          fournisseurId: data.fournisseur?.id || '',
          fournisseurLabel: data.fournisseur
            ? (data.fournisseur.entreprise || `${data.fournisseur.nom} ${data.fournisseur.prenom || ''}`.trim())
            : '',
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link href={`/achats/${id}`} className="p-2 rounded-xl hover:bg-gray-100 transition-colors">
